...
- [**11. 命令行接口(CLI)**](#11-命令行接口cli)
    ...
    - [**11.2 CLI 示例**](#112-cli-示例)
        - [**11.2.1 示例 1：创建网络**](#1121-示例-1创建网络)
        - [**11.2.2 示例 2：发送属性读取**](#1122-示例-2发送属性读取)
        - [**11.2.3 示例 3：发送簇命令**](#1123-示例-3发送簇命令)
...
1. 使用 **client-command-macro.h** 文件中的填充宏构造命令：
    ...
    **identifyTime** 是规范中定义的 int16u，是设备应继续识别自身的秒数。
...
1. 使用 **app/framework/gen/client-command-macro.h** 中包含的命令响应宏填充响应命令缓冲区，例如：
    ...
    timeout 是一个 int16u，表示设备将继续识别自身的秒数。
