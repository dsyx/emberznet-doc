...
3. 制造商特定的命令（Manufacturer-Specific Commands）：可以通过向该簇添加制造商特定的命令来扩展标准 ZigBee 簇。标准 ZigBee 簇内的制造商特定的命令可以使用整个范围（命令 ID：0x00-0xff）。必须为制造商特定的命令提供双字节制造代码，以便可以将该命令与该簇中的标准 ZigBee 命令区分开。
    1. 示例：在应用框架附带的 **sample-extensions.xml** 文件中，我们定义了三个命令来扩展 On/Off 簇，名为 OffWithTransition、OnWithTransition 和 ToggleWithTransition。这些命令与该簇中的标准 Off、On 和 Toggle 命令共享相同的命令 ID。但是，它们还包含制造商代码 0x1002，表明它们是 Silicon Labs 的制造商特定命令。
4. 制造商特定的属性（Manufacturer-Specific Attributes）：可以通过向应用程序添加制造商特定的属性来扩展标准 ZigBee 簇。标准 ZigBee 簇中制造商特定的属性可以使用整个地址空间（属性 ID：0x0000 ~ 0xffff）。必须为每个制造商特定的属性包含双字节制造商代码，以便可以将其与非制造商特定属性区分开来。
    1. 示例：在应用框架附带的 **sample-extensions.xml** 文件中，我们定义了一个属性 Transition Time，它与 on/off 簇 0x0000 中的 on/off state 共享相同的属性 ID。但是，transition time 属性还包含制造商代码 0x1002，表明它是 Silicon Labs 的制造商特定属性。

> Note：Silicon Labs 的制造商代码 0x1002 由 ZigBee 组织定义，并包含在制造商代码数据库中（ZigBee document #053874）。制造商特定的簇、属性和命令的实现需要制造商代码。ZigBee 为每个请求组织提供唯一的制造商代码。要获得贵组织的制造商代码，请通过 [http://zigbee.org](http://zigbee.org) 与 ZigBee 联系。

## **16.2 局限性考虑**

在使用制造商特定的簇、属性和命令扩展应用框架时，需要考虑两个值得注意的限制。

* 所有簇 ID（包括制造商特定簇的 ID）必须在单个设备中是唯一的。Ember 应用框架当前不支持在单个设备中重叠制造商特定的簇 ID。换句话说，您无法在同一设备上实现簇 0xFC00 （制造商代码 0xFEED）和簇 0xFC00（制造商代码 0xBEEF）。Ember 应用框架假定所有簇 ID 都是唯一的，无论与它们相关联的制造商代码如何。
* 制造商特定簇中的所有属性和命令 ID 必须是唯一的，并且假定它们与它们所在的簇具有相同的制造商代码。ZigBee 协议不支持在制造商特定簇中重叠制造商特定的属性或命令 ID（具有不同的制造商代码）。原因是只有一个制造商代码在 ZigBee 应用程序头中传递。如果所寻址的簇在制造商特定的范围 0xFC00 - 0xFFFF 中，则假定制造商代码适用于簇。This makes it impossible to address, for instance, Attribute 0x0000 with manufacturer code 0xFEED inside cluster 0x0000 with manufacturer code 0xBEEF. The Ember application framework does not even bother to store individual manufacturer codes for attributes within a manufacturer-specific cluster since the manufacturer code of the cluster is assumed to apply to all of the attributes within it.

## **16.3 在应用框架和 Ember AppBuilder 内定义 ZCL 扩展**

整个 ZigBee 簇库在 **tool/appbuilder** 目录中以 XML 格式定义。除了描述 Ember 应用框架使用的标准 ZCL 的簇、命令和属性的预期 XML 文件（如 general.xml 或 ha.xml）外，还有一个示例扩展文件 **sample-extensions.xml**。此 XML 文件包含多个示例 ZigBee 扩展，包括自定义簇、添加到 on/off 簇的自定义属性、添加到 on/off 簇的自定义命令。

为了扩展 ZigBee 簇库，您必须为您的扩展创建类似的扩展文件，并按照文档 **UG112：Ember AppBuilder User’s Guide** 和 Ember AppBuilder 在线帮助中标题为 “Creating Custom Clusters” 的说明将其添加到 Ember AppBuilder 中。

您可以使用 Ember 应用框架和 Ember AppBuilder 的 XML Schema Definition 文件（XSD）验证您的自定义 XML 配置文件。您可以通过转到 **File | Preferences | App Builder | ZCL Application** 并单击位于可用栈配置列表下的 **export XSD file**，来生成适用于您特定 Ember Desktop 版本的 **appbuilder.xsd**。有关扩展 Ember 应用框架的更多文档包含在 **sample-extensions.xml** 文件中。

> Note：Any multi-byte numeric constant values specified in the XML file should specify the full number of digits as hex, such as "0x000000000000" (for an int48u) rather than simply "0x00" or "0". This ensures that the proper default value will be added to the GENERATED\_DEFAULTS define in the \<appname\>\_endpoint\_config.h file during generation.

## **16.4 制造商特定的属性 API**

用于与属性交互的 Ember 应用框架中的一些 API 已被修改，其将制造商代码作为参数。

### **16.4.1 属性读写**

所有读写属性的函数都具有附加功能，这些功能将制造商代码与其余属性寻址信息一起使用。当您读写制造商特定的属性时，您必须提供要读写的属性的制造商代码，以便可以在属性表中找到它。例如，您可以使用函数 **emberAfReadServerAttribute** 读取标准 ZigBee 属性。然而，如果为制造商特定的属性调用此函数，则没有制造商代码参数允许您正确识别制造商特定的属性，因此读取将失败。如果要读取制造商特定的属性，则必须使用制造商特定的函数 **emberAfReadManufacturerSpecificServerAttribute** 和 **emberAfReadManufacturerSpecificClientAttribute**。这两个函数都要求制造商代码，然后将它们传递给通用函数 **emberAfReadOrUpdateAttribute**。
...
我们还在 Ember 应用框架中添加了制造商特定的属性更改回调，因此标准属性回调不需要浪费代码空间来检查不存在的制造商代码。
